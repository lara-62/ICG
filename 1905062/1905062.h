...
unsigned long long sdbm_hash(string str)
{
    ...
    void varCount(int count)
    {
        number_of_variable+=count;
    }
    int getvarCount()
    {
...
void varCountCurrentScope(int count)
{
    current_scope->varCount(count);
}
int getvarCountCurrentScope()
{
    return current_scope->getvarCount();
}
};